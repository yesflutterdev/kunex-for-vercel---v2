...
// Static method to get user's favorites with filtering and sorting
...
// Static method to check if business is favorited by user
favoriteSchema.statics.isFavorited = function(userId, businessId) {
  return this.findOne({ userId, businessId }).lean();
};
...
favoriteSchema.statics.getUpcomingReminders = function(userId, days = 7) {
  ...
  .populate({
    path: 'businessId',
    select: 'businessName logo location contactInfo'
  })
